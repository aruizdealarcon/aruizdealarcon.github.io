...
--[[
    ...
    Copyright (C) 2016  Joseph Rabinoff. Modified by Alberto Ruiz de Alarcon
...
label = ">> TikZ Export Modified"

methods = {
   { label="Export selection to clipboard", run=run },
   --{ label="Export selection to textbox", run=run },
...
\tikzstyle{tensor-tiny} = [inner sep=1pt]
...
resfactor = 50 -- Shape resize factor
...
function copy_to_clipboard(text)
    local pipe = popen('pbcopy', 'w')  -- Abre el comando 'pbcopy' para escritura
    if pipe then
        pipe:write(text)  -- Escribe el texto en el comando 'pbcopy'
        pipe:close()  -- Cierra el pipe, lo que envía el texto al portapapeles
    else
        print("Error: No se pudo acceder al portapapeles.")
    end
end

function copy_to_clipboard_multi(text)
    -- Determinar el sistema operativo usando el comando 'uname'
    local os_pipe = popen("uname", "r") -- 'r' para leer la salida del comando
    local os_output = os_pipe:read("*a") -- Leer toda la salida como una cadena
    os_pipe:close()

    local command

    -- Seleccionar el comando adecuado según el sistema operativo
    if os_output:match("Linux") then
        -- En Linux, intentar con xclip o xsel
        if execute("command -v xclip") then
            command = 'xclip -selection clipboard'
        elseif execute("command -v xsel") then
            command = 'xsel --clipboard --input'
        else
            print("Error: xclip o xsel no está instalado.")
            return
        end
    elseif os_output:match("Darwin") then
        ...
        -- Intentar con 'clip' para Windows, aunque esto requiere que el script se ejecute en un entorno donde 'uname' no devuelva nada.
        command = 'clip'
    end

    -- Ejecutar el comando correspondiente para copiar al portapapeles
...
-- Convert a number to a string, omitting trailing zeros after the decimal, and rounding. For angles only!
...
function export_mark(model, obj, matrix)
   ...
   if strocol ~= "turquoise" and fillcol ~= "turquoise" then
      write(indent .. "\\node")
      if #options > 0 then
...
function export_curve(subpath, matrix, options)
   ...
   then
      ...
      then -- It's a rectangle
         ...
         if translate_origin then
            ...
            ret = ret .. newline .. pathindent .. "(0, 0)"
         else
            ret = ret .. newline .. pathindent .. svec(matrix*pt1)
         end
      ...
      if segment.type == "segment" then
         if i == 1 then
            ...
            if translate_origin then
               ...
               ret = ret .. newline .. pathindent .. svec(matrix*segment[1])
            end
         end
         ret = ret .. newline .. pathindent .. " -- " .. svec(matrix*segment[2])
         ...
         for j,bezier in ipairs(beziers) do
            if i == 1 and j == 1 then
               ...
               if translate_origin then
                  ...
                  ret = ret .. newline .. pathindent .. svec(matrix*bezier[1])
               end
            end
            ret = ret .. newline .. pathindent .. " .. controls "
      ...
      elseif segment.type == "arc" then
         ...
         if i == 1 then
            ...
            if translate_origin then
               ...
               ret = ret .. newline .. pathindent .. svec(matrix*beginp)
            end
         end
   ...
   if subpath.closed then
      ...
      ret = ret .. newline .. pathindent .. " -- cycle"
   end
...
function export_path(shape, mode, matrix, obj)
   ...
   if obj then
      ...
      if drawing then
         ...
         -- end
         -- need draw= if filling
      	 if strocol == "darkorange" then
           color_option("virtual", "draw", options, nil, not filling)
         elseif strocol == "orange" then
            color_option("virtual", "draw", options, nil, not filling)
         elseif strocol == "red" then
            color_option("virtual", "draw", options, nil, not filling)
